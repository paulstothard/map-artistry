...
def main():
    parser = argparse.ArgumentParser(description="Pure-Python DEM prep for a GeoJSON boundary.")
    parser.add_argument(
        '-b', '--boundary', type=Path, required=True,
        help='Input GeoJSON boundary'
    )
    parser.add_argument(
        '-o', '--output', type=Path, required=True,
        help='Output clipped DEM GeoTIFF'
    )
    ...
    print(f"[ ] Determining tiles for bounds: {minx:.4f},{miny:.4f} — {maxx:.4f},{maxy:.4f}")
